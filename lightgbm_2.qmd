...
pct_children_df <- hoteles_df |> group_by(country) |> summarise(pct_children = mean(children)*100)

hoteles_df <- hoteles_df |> mutate(children = as.factor(children))

hoteles_df <- left_join(x=hoteles_df, y=pct_children_df, by="country")
...
receta_hoteles <- recipe(children ~ 
           ...
           country_code + pct_children + is_repeated_guest +
  ...
  step_cut(stays_in_week_nights, breaks = c(0, 1, 2, 3, 4, 5), 
           include_outside_range = TRUE) |>
  step_mutate(no_company = ifelse(company == "NULL", 1, 0)) |>
  ...
  step_mutate(waiting_list = ifelse(days_in_waiting_list == 0, 0, 1)) |>
  step_mutate(previous_cancellations = ifelse(previous_cancellations == 0, 0, 1)) |>
  step_mutate(previous_bookings_not_canceled = 
                ifelse(previous_bookings_not_canceled == 0, 0, 1)) |>
  step_relevel(meal, ref_level = "Undefined", skip=TRUE) |>
  step_mutate(market_segment = ifelse(market_segment=="Direct", 1, 
                                      ifelse(market_segment == "Online_TA", 1, 
                                      ifelse(market_segment == "Offline_TA/TO", 1, 0)))) |>
  step_mutate(customer_type = ifelse(customer_type == "Transient", 1, 
                                     ifelse(customer_type == "Transient-Party", 1, 0))) |>
  ...
  step_select(-company, skip = TRUE) |>
  step_select(-days_in_waiting_list, skip = TRUE) |>
  ...
  step_interact(terms = ~ hotel:starts_with("waiting_list")) |>
  ...
  step_interact(terms = ~ adults:starts_with("no_company")) |>
  step_interact(terms = ~ adults:starts_with("waiting_list")) |>
  step_interact(terms = ~ adults:starts_with("previous_cancellations")) |>
  step_interact(terms = ~ adults:starts_with("previous_bookings_not_canceled")) |>
  ...
  step_interact(terms = ~ lead_time:starts_with("no_company")) |>
  step_interact(terms = ~ lead_time:starts_with("previous_cancellations")) |>
  step_interact(terms = ~ lead_time:starts_with("previous_bookings_not_canceled")) |>
  step_interact(terms = ~ lead_time:starts_with("waiting_list")) |>
  ...
  step_interact(terms = ~ average_daily_rate:starts_with("no_company")) |>
  step_interact(terms = ~ no_company:starts_with("customer_type")) |>
  step_interact(terms = ~ no_company:starts_with("reserved_room_type")) |>
  step_interact(terms = ~ no_company:starts_with("market_segment")) |>
  step_interact(terms = ~ no_company:starts_with("is_prt")) |>
  step_interact(terms = ~ no_company:starts_with("total_of_special_requests")) |>
  step_interact(terms = ~ no_company:starts_with("waiting_list")) |>
  step_interact(terms = ~ no_company:starts_with("previous_cancellations")) |>
  step_interact(terms = ~ no_company:starts_with("previous_bookings_not_canceled")) |>
  ...
  step_interact(terms = ~ is_prt:starts_with("waiting_list")) |>
  step_interact(terms = ~ is_prt:starts_with("previous_cancellations")) |>
  step_interact(terms = ~ is_prt:starts_with("previous_bookings_not_canceled"))
```
...
hoteles_test <- left_join(x=hoteles_test, y=pct_children_df, by="country")
...
write.csv(res,"lightgbm_log_loss_2.csv", row.names = FALSE)
```
